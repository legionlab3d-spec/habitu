...
export default async function DocumentosResidentePage() {
  ...
  const lista = await getDocumentos(supabase, perfil.conjunto_id, true)
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 max-w-2xl">
      <div className="mb-6 md:mb-8">
        ...
        <p className="text-sm text-[#3f4948] mt-1">Documentos del conjunto</p>
      </div>

      {conUrls.length === 0 ? (
        <div className="bg-[#f5f3f3] border border-[#bec9c8] rounded-2xl p-8 text-center">
          <p className="text-[#3f4948] text-sm font-medium">No hay documentos publicados aún.</p>
        </div>
      ) : (
            ...
            <div key={d.id} className="bg-white border border-[#bec9c8] rounded-2xl px-4 py-3 flex items-center justify-between gap-4">
              ...
              {d.signed_url && (
                <a
                  ...
                  className="text-xs text-[#004746] border border-[#004746] hover:bg-[#004746] hover:text-white transition-colors px-3 py-1.5 rounded-lg flex-shrink-0"
                >
                  Ver
                </a>
              )}
